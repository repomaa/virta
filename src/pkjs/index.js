...
function fetchAndSend(region) {
  var url = "https://api.spot-hinta.fi/TodayAndDayForward?region=" + encodeURIComponent(region) + "&priceResolution=60";
  ...
  xhr.onreadystatechange = function() {
    ...
    try {
      ...
      var now = new Date();
      var currentHourStart = new Date(now.getFullYear(), now.getMonth(), now.getDate(), now.getHours(), 0, 0, 0);
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        var itemHourStart = new Date(dt.getFullYear(), dt.getMonth(), dt.getDate(), dt.getHours(), 0, 0, 0);
      ...
      for (var j = 0; j < futureItems.length; j++) {
        ...
        if (p == null) {
          ...
          encoded.push(String(Math.round(p * 1000000)));
        }
      }
